...
def InvMix(column):
    new_col = [0 for _ in range(4)]
    for k in range(0,4):
        #for each column in the Galois Field polynomial b(x)
        for j, entry in enumerate(column):
            #Grab the current iteration of b(x)
            val = B_X[3-k][j]
            #multiple the values in b(x) and our column vector in the Field G(2^8)
            res = int(hex(fieldMult(entry,val))[2:], 16)
            #XOR the result into the new column vector
            new_col[k] ^= res
