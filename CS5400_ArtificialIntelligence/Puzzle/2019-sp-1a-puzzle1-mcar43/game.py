...
class State:
    ...
    def findWrigglers(self):
        wrigglerDict = [Wriggler() for i in range(self.numWrigglers)]
    ...
    def redrawState(self):
        ...
        for i in range(self.numWrigglers):
            current = self.wrigglers[i].head
            while current.nextSegment:
            ...
            for key in DIRECTIONS:
                ...
                if nextRow in range(0, self.rows) and nextCol in range(0, self.cols):
                    ...
                    if self.board[nextRow][nextCol] == 'e':
                        ...
                        if direction > 0:
                            OUTPUT_FORMAT.append([id, 0, nextRow, nextCol])
                        else:
                            OUTPUT_FORMAT.append([id, 1, nextRow, nextCol])

        return VALID_MOVES, OUTPUT_FORMAT

    def isGoalState(self):
        blueWriggler = self.wrigglers[0].head
        headRow, headCol = blueWriggler.row, blueWriggler.col
        if (headRow + 1) == self.rows and (headCol + 1) == self.cols:
            return True
...
class Graph:
    ...
    def addVertex(self, root, vertex):
        for key in self.vertices:
            if vertex.state.board == self.vertices[key].state.board:
                #self.addEdge(root, self.vertices[key])
                return False
    ...
    def display(self):
        for key in self.vertices:
            print("Depth: %d Vertex: %s -> %s" % (self.vertices[key].depth, key, [ident.ident for ident in self.vertices[key].connected]))
