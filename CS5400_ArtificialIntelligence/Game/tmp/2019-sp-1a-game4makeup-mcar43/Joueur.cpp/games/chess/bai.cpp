...
std::string AI::make_move()
{
  ...
  //  std::cout << "Starting MMAX with root state of: \n";
   // currentState.print(currentState._all);
    //mmax(bestMove, currentState, 3, true, MAX_VALUE, MIN_VALUE);
    std::cout << "Current State:\n";
    currentState.print(currentState._all);
    currentState.filterLegal();
    std::cout << "Has children: \n";
    for(std::vector<Bitboard>::iterator it = currentState._children.begin(); it != currentState._children.end(); ++it) {
      std::cout << it->assocMove.SAN() << " - ";
    }
    mmax(bestMove, currentState, 3, true, MIN_VALUE, MAX_VALUE);
...
int AI::mmax(Move & bestMove, Bitboard & _game, int depth, bool maximizer, int alpha, int beta) {
  bool debug = true;
  if(debug) { 
    std::cout << "\nEntering Minimax With the following paramters:\n";
    std::cout << "Depth: " << depth << std::endl;
    std::cout << "BestMove: " << bestMove.SAN() << " eval of: " << bestMove._eval << std::endl;
    std::cout << "GameMove: " << _game.assocMove.SAN() << std::endl;
    std::cout << "Alpha: " << alpha <<  std::endl;
    std::cout << "Beta: " << beta << std::endl;
  }
  if(depth == 0) {
    _game.evalBoard();
    return _game.assocMove._eval; 
  }
  if(maximizer) {
    if(debug) {
      std::cout << "Entered into MAX\n";
    }
    int max = MAX_VALUE;
    _game.filterLegal();
    Bitboard newMove = Bitboard(_game);
    for(std::vector<Bitboard>::iterator _child = newMove._children.begin(); _child != newMove._children.end(); ++_child) {
      int val = mmax(bestMove, *_child, depth - 1, !maximizer, alpha, beta);
      if(debug) {
        std::cout << "Evaluation: " << val << std::endl;
        std::cout << "Max: " << max << std::endl;
        std::cout << "BestMove: " << bestMove._eval << std::endl;
      }
      if (val > max) {
        max = val;
        bestMove = _game.assocMove;
      }
