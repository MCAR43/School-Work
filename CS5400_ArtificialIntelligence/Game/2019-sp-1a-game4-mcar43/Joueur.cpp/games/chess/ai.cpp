...
std::string AI::make_move()
{
    ...
    currentState.allMoves(false);
    currentState.filterLegal();
    for(std::vector<Move>::iterator it = currentState._moveList.begin(); it != currentState._moveList.end(); ++it) {
      if(it->_eval > bestMove._eval) {
        bestMove = (*it);
      } 
    }
